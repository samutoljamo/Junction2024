...
function gpsToNormalized(
    ...
    const top_left = { lat: 60.162410, lon: 24.905575 } // Top-left corner
    const top_right = { lat: 60.161860, lon: 24.905884 } // Top-right corner
    ...
    const y = Math.sqrt((lat - top_left.lat) ** 2 + (lon - top_left.lon) ** 2) / Math.sqrt((bottom_left.lat - top_left.lat) ** 2 + (bottom_left.lon - top_left.lon) ** 2);
    const x = Math.sqrt((lat - top_left.lat) ** 2 + (lon - top_left.lon) ** 2) / Math.sqrt((top_right.lat - top_left.lat) ** 2 + (top_right.lon - top_left.lon) ** 2);
    return { x, y };
